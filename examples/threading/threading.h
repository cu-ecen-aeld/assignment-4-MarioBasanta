 ...
 */
struct thread_data {
    pthread_mutex_t *mutex;         // Pointer to mutex to lock/unlock
    int wait_to_obtain_ms;          // Milliseconds to wait before locking
    int wait_to_release_ms;          // Milliseconds to hold mutex before unlocking
    bool thread_complete_success;  // Success flag
};
